...
- Read enough of a file to fully understand it before editing. For normal source files, read the whole file — otherwise you don’t know what you don’t know, and will end up making mistakes, duplicating code that already exists, or misunderstanding the architecture. For very large or generated files (lockfiles, build output, huge data files), read the relevant sections plus enough surrounding context to avoid duplicating or breaking existing code.
...
- Your internal knowledgebase of libraries might not be up to date. When working with any external library, unless you are 100% sure that the library has a super stable interface, you will look up the latest syntax and usage via web search (and the library’s official documentation) rather than relying on your internal knowledge
...
- For every task, get a lightweight plan approved in chat before writing code (see below). Only create persistent PLAN.md / STATUS.md files for larger efforts that cross the Task Breakdown Thresholds (>5 file changes); smaller tasks don't need them. If PLAN.md already exists, read it and prioritize the plan it gives, and keep it up to date as features are planned, implemented, and completed.
...
Only for efforts that cross the Task Breakdown Thresholds (>5 file changes). Adapt sections based on project type. Core sections always required:
