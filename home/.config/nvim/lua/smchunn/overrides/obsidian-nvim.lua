local FzfPicker = require("obsidian.pickers._fzf")
...
---@param opts { callback: fun(path: string)|?, no_default_mappings: boolean|?, selection_mappings: obsidian.PickerMappingTable|?, query_mappings: obsidian.PickerMappingTable|?  }
...
local function get_value_actions(display_to_value_map, opts)
	...
	local actions = {
		default = function(selected)
			...
			-- If user passed PickerEntry, unpack the `.path`
			local unpacked_values = vim.tbl_map(function(v)
				if type(v) == "table" and v.path and type(v.path) == "string" then
					return v.path
				else
					return v
				end
			...
			opts.callback(unpack(unpacked_values))
		end,
	}
