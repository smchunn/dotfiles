...
set --query hydro_color_git_branch || set --global hydro_color_git_branch $hydro_color_pwd
set --query hydro_color_git_branch_icon || set --global hydro_color_git_branch_icon brgreen
set --query hydro_color_git_dirty || set --global hydro_color_git_dirty brred
set --query hydro_color_git_ahead || set --global hydro_color_git_behind brblue
set --query hydro_color_git_behind || set --global hydro_color_git_behind green
...
function _hydro_prompt --on-event fish_prompt
  ...
  fish --private --command "
    set branch \"$_hydro_color_git_branch_icon$hydro_symbol_git_branch $_hydro_color_git_branch\$(command git symbolic-ref --short HEAD 2>/dev/null)\"

    test -z \"\$$_hydro_git\" && set --universal $_hydro_git \"\$branch \"

    command git diff-index --quiet HEAD 2>/dev/null
        test \$status -eq 1 ||
            count (command git ls-files --others --exclude-standard (command git rev-parse --show-toplevel)) >/dev/null && set info \"$_hydro_color_git_dirty$hydro_symbol_git_dirty\"

    for fetch in $hydro_fetch false
      command git rev-list --count --left-right @{upstream}...@ 2>/dev/null | read behind ahead

      test \$ahead -gt 0 && set upstream \"$_hydro_color_git_ahead\$ahead$hydro_symbol_git_ahead\"
      test \$behind -gt 0 && set upstream \"\$upstream$_hydro_color_git_behind\$behind$hydro_symbol_git_behind\"
...
for color in hydro_color_{pwd,git,git_branch,git_branch_icon,git_dirty,git_ahead,git_behind,error,prompt,duration,start}
...
function hydro_left
  ...
  echo -e "$lprompt$hydro_color_normal"
end

function hydro_right
  set -l rprompt "$_hydro_color_pwd$_hydro_pwd"
  set --query _hydro_git && string length -- $_hydro_git &>/dev/null && set -l rprompt "$rprompt $$_hydro_git"
  set -l rprompt "$rprompt$hydro_color_normal"
